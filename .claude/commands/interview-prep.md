---
description: Kick off your interview preparation — builds a custom prep plan, STAR story bank, mock interviews, and delivery coaching
argument-hint: [job-slug (optional)]
allowed-tools: [Task, Read, Write, Edit, Glob]
---

You are kicking off interview preparation for a Career Architect user.

## Pre-Flight

1. Verify user profile: `profile/profile.md`
...
1. **Intake** — The user profile and job analysis brief are pre-loaded. Do not re-ask anything already answered by those files. Only confirm details that are genuinely unknown:
...
2. **Risk Assessment** — Immediately identify the user's top 3 preparation risks based on their profile:
...
5. **Mock Interview** — Run at least one timed mock during this session:
   - Select question type based on the user's highest-risk area
...
6. **Company Research Brief** — If target company is known, generate:
   ...
   - Recommended questions the user should ask the interviewer
...
> 2. Run `/interview-prep` again for another mock round
